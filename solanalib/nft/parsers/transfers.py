...
def parse_transfer_type_transfer(
    ...
    def is_owner_of_account(owner: str, token_account: str) -> bool:
        if not owner:
            return False
    ...
    def get_initial_authority_of_token_account(new_token_account: str):
        ...
        for post_owner in tx.get_all_post_owners(mint):
            if is_owner_of_account(owner=post_owner, token_account=new_token_account):
    ...
    def get_new_authority_of_token_account(new_token_account: str):
        ...
        init_authority = get_initial_authority_of_token_account(new_token_account)
    ...
    def get_old_authority_of_token_account(
        ...
        for pre_owner in tx.get_all_pre_owners(mint):
            if is_owner_of_account(owner=pre_owner, token_account=old_token_account):
    ...
    def parse_transfer_ix(ix: Instruction) -> Union[TransferActivity, None]:
    ...
    for index, ix in enumerate(tx.instructions.outer):
        ...
        transfer_activity = parse_transfer_ix(ix=ix)
        ...
        if not ix.is_parsed:
            ...
            for iindex, iix in enumerate(tx.instructions.inner[index]):
                ...
                transfer_activity = parse_transfer_ix(ix=iix)
                if transfer_activity:
                    transfers.append(transfer_activity)
