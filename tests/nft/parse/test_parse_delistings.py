...
class TestParseDelistings:
    def test_parse_delisting_mev1_01(self, load_nft_tx):
        tx = load_nft_tx("delistings", "mev1_01")
    ...
    def test_parse_delisting_mev2_01(self, load_nft_tx):
        tx = load_nft_tx("delistings", "mev2_01")
        mint = "Ge2L2Bt8CPsVEFRZBKSu5dCnz746i7ukbBCpAsPv44VL"
        activity = parse_delisting(tx=tx)
        assert isinstance(activity, DelistingActivity)
        assert activity.mint == mint
    ...
    def test_parse_delisting_mev2_02(self, load_nft_tx):
        tx = load_nft_tx("delistings", "mev2_02")
        mint = "Ge2L2Bt8CPsVEFRZBKSu5dCnz746i7ukbBCpAsPv44VL"
        activity = parse_delisting(tx=tx)
        assert isinstance(activity, DelistingActivity)
        assert activity.mint == mint
    ...
    def test_parse_delisting_mev2_03(self, load_nft_tx):
        tx = load_nft_tx("delistings", "mev2_03")
