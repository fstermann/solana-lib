...
class TestParseTransfers:
    def test_parse_transfer_outer_inner_01(self, load_nft_tx):
        # Transfers twice in 1 tx, once in the outer ix, once in the inner
        tx = load_nft_tx("transfers", "outer_inner_01")
    ...
    def test_parse_transfer_transferChecked_01(self, load_nft_tx):
        # Is transferred by transferChecked
        # the new_authority is not mentioned in the tx
        tx = load_nft_tx("transfers", "transferChecked_01")
    ...
    def test_parse_transfer_transferChecked_02(self, load_nft_tx):
        # Is transferred by transferChecked
        # the new_authority is mentioned in the tx post/pre token balances
        tx = load_nft_tx("transfers", "transferChecked_02")
        mint = "Ge2L2Bt8CPsVEFRZBKSu5dCnz746i7ukbBCpAsPv44VL"
        activity = parse_transfer(tx=tx, mint=mint)
        assert isinstance(activity, TransferActivity)
        assert activity.mint == mint
    ...
    def test_parse_transfer_outer_setAuthority(self, load_nft_tx):
        # Transfers once in outer ix, new authority is set
        tx = load_nft_tx("transfers", "outer_setAuthority")
    ...
    def test_parse_transfer_inner_01(self, load_nft_tx):
        # Transfers once in inner ix
        tx = load_nft_tx("transfers", "inner_01")
    ...
    def test_parse_transfer_inner_multisigAuthority(self, load_nft_tx):
        # Transfers once in inner, transfer is signed by multisig
        # previous owner receives sol from close account
        tx = load_nft_tx("transfers", "inner_multisigAuthority")
        mint = "FPJtQasfsUmjsJ9pmYVPReNpXMBkVpqavnrcFiWdrx5A"
        activity = parse_transfer(tx=tx, mint=mint)
        assert isinstance(activity, TransferActivity)
        assert activity.mint == mint
    ...
    def test_parse_transfer_inner_without_init_or_create(
        self, mock_Client_get_account_info, load_nft_tx, load_account_info
    ):
        ...
        tx = load_nft_tx("transfers", "inner_without_init_or_create")
    ...
    def test_parse_transfer_unknown(self, load_nft_tx):
        # Transfers unknown
        tx = load_nft_tx("transfers", "unknown")
        mint = "4mSvhZ4CCWixrYkwHjL6yVCoURn2U8xoXN7zbpakzMck"
        activity = parse_transfer(tx=tx, mint=mint)

        assert isinstance(activity, TransferActivity)

    def test_parse_transfer_unknown02(self, load_nft_tx):
        # Transfers unknown
        tx = load_nft_tx("transfers", "unknown02")
        mint = "4mSvhZ4CCWixrYkwHjL6yVCoURn2U8xoXN7zbpakzMck"
        activity = parse_transfer(tx=tx, mint=mint)
